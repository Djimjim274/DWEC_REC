...
let nombre =entrada.map(entrada=>entrada.nombre);
 nombre=nombre.map(valor=>valor.toLowerCase()) //pasamos a minusculas
//pasamos el nombre sin tilde
...
//dividimos el nombre apellido1 y apellido2
 nombre=nombre.map(valor=>valor.split(' '));
 
//la última letra (sin tilde) del nombre
...
// las tres primeras del segundo apellido (minúsculas y sin tildes)
  ape2=nombre.map(valor=>valor[2].substring(0,3)) 
//los dígitos que ocupan las posiciones 3,4 y 5del DNI
let dni =entrada.map(valor=>valor.DNI)
...
/* const edad=(edad)=>{
    let fecha = new Date();
    let year=year.getFullYear(); console.log(year);

}

console.log(edad('21/04/2002'))
